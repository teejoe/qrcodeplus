...
public class WeakFinderPatternFinder extends BaseFinderPatternFinder {
     ...
     */
    public WeakFinderPatternFinder(BitMatrix image) {
        this(image, null);
    }
    ...
    public final FinderPatternInfo find(Map<DecodeHintType, ?> hints) throws NotFoundException {
        ...
        if (decodeState != null) {
            ...
            if (decodeState.specifiedParams != null) {
                ...
                decodeState.previousFailureHint.weakFinderPatternFinderHint.sensitivityIncrease = sensitivityIncrease;
            } else {
        ...
        // 1:3:1 ratio; this tracks the number of such modules seen so far
        ...
        for (int i = iSkip - 1; i < maxI && !done; i += iSkip) {
            ...
            for (int j = 0; j < maxJ; j++) {
                if (image.get(j, i)) {
                    // Black pixel
                    if ((currentState & 1) == 0) { // Counting white pixels
                    ...
                    if ((currentState & 1) == 1) { // Counting black pixels
                        currentState++;
                    }
                    stateCount[currentState]++;
                }
    ...
    /**
     * Given a count of white/black/white pixels just seen and an end position,
    ...
    protected boolean foundPatternCross(int[] stateCount) {
        ...
        return
                Math.abs(moduleSize - stateCount[0]) < maxVariance &&
                        Math.abs(3.0f * moduleSize - stateCount[1]) < 3 * maxVariance &&
